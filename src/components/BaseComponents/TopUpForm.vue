<script setup lang="ts">
  ...
  const getPreviewContent = () => {
    ...
    return {
      title: 'Rioni Capital — Реквизиты для пополнения',
...
<template>
  ...
  <v-dialog v-model="previewDialog" :width="mobile ? 'auto' : 700" :min-width="mobile ? '100%' : 700">
    <v-card>
      <v-sheet class="modal-window pa-4">
        <v-card-title class="d-flex justify-center align-center">
          <v-sheet class="font-semibold font-22 text-background-blue">
            {{ t('topUpAccount.previewTitle') || 'Предпросмотр реквизитов' }}
          </v-sheet>
        ...
        <v-card-text class="mt-4">
          <v-sheet class="preview-content pa-4" style="background: white; border-radius: 8px;">
            ...
            <v-sheet class="font-semibold mb-2" style="border-bottom: 1px solid #ddd; padding-bottom: 5px;">
              Информация о платеже
            </v-sheet>
            <v-sheet class="d-flex ga-4 mb-4">
              ...
              <v-sheet>
                ...
                <span class="font-weight-bold ml-2">{{ getPreviewContent().amount || '100' }} {{ getPreviewContent().currencySymbol }}</span>
              </v-sheet>
            </v-sheet>
            ...
            <v-sheet class="d-flex flex-column ga-1 mb-4">
              <v-sheet><span style="color: #666;">IBAN:</span> <span class="font-weight-bold font-monospace">{{ getPreviewContent().iban }}</span></v-sheet>
              <v-sheet><span style="color: #666;">Банк:</span> <span class="font-weight-bold">{{ getPreviewContent().bankName }}</span></v-sheet>
              <v-sheet><span style="color: #666;">SWIFT/BIC:</span> <span class="font-weight-bold font-monospace">{{ getPreviewContent().swiftBic }}</span></v-sheet>
              <v-sheet><span style="color: #666;">Страна:</span> <span class="font-weight-bold">{{ getPreviewContent().country }}</span></v-sheet>
              <v-sheet><span style="color: #666;">Адрес:</span> <span class="font-weight-bold">{{ getPreviewContent().address }}</span></v-sheet>
            </v-sheet>
            ...
            <v-sheet class="pa-3 rounded" style="border: 1px solid #d4edda; background: #f0f8f0; font-size: 11px; color: #666;">
              <strong>Внимание!</strong> При пополнении счета с реквизитов не указанных в личном кабинете, процесс пополнения счета может занять больше времени из-за дополнительной проверки.
            </v-sheet>
          </v-sheet>
  ...
  <v-dialog v-model="pdfPreviewDialog" :fullscreen="true">
    <v-card>
      <v-sheet class="d-flex justify-space-between align-center pa-2" style="background: #103673; color: white;">
      ...
      <v-sheet class="d-flex ga-2 pa-3" style="background: white;">
        <v-btn variant="flat" rounded="ml" class="btn-custom flex-grow-1" @click="sharePDF(); closePdfPreview();">
